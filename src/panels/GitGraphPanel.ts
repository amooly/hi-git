...
export class GitGraphPanel {
    ...
    public static createOrShow(extensionUri: vscode.Uri, targetUri?: vscode.Uri) {
        ...
        if (GitGraphPanel.currentPanel) {
            GitGraphPanel.currentPanel._targetUri = targetUri;
        ...
        GitGraphPanel.currentPanel = new GitGraphPanel(panel, extensionUri, targetUri);
    }
    ...
    private _update() {
        ...
        this._panel.title = this._targetUri ? `Git Graph: ${path.basename(this._targetUri.fsPath)}` : 'Git Graph';
