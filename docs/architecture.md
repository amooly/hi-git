...
│   └── frontend/                 # Webview React source (authored as JSX, excluded from tsc)
...
Every `<script>` tag requires a per-load nonce. The nonce is generated by `src/utilities/getNonce.ts` and applied by both panel providers.
